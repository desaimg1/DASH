...
def test_underlay_bgp(confgen, dpu, dataplane):
    ...
    # STEP1 : Configure DPU
    with (current_file_dir / 'dpu_json/test_bgp_setup_commands.json').open(mode='r') as config_file:
    ...
    result = dataplane.check_bgp_neighborship_established()
    ...
    assert(result is True, "Test underlay bgp FAILED!!")